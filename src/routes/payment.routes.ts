...
const router = Router();
// Guest flows
router.post('/deposit/initiate',        requireGuestAuth, requireVenueFeature('preOrder'), paymentLimiter, Payment.initiateDeposit);
router.post('/deposit/capture',         paymentLimiter, Payment.captureDeposit);
router.post('/final/initiate',          requireGuestAuth, requireVenueFeature('finalPayment'), paymentLimiter, Payment.initiateFinalPayment);
